...
- **Persistence is not wired to the API.** The schema, RLS and isolation tests
  exist and pass; the sqlx repository layer and the endpoints that would use it
  do not. `POST /v1/analyses` runs the pipeline in-process and returns the result
  without storing it.
- **No object storage.** Documents are analysed from the request body. Immutable
  storage, hashing and signed URLs are modelled in `skattjakt-core::document` and
  in the schema, but nothing writes to a bucket yet.
- **No OCR.** See D12.
- **No UI.** The API is the only surface.
- **Docker and Kubernetes are unverified.** The manifests are written; the
  container daemon and `kubectl` were unavailable in the environment that built
  this, so neither has been run.
