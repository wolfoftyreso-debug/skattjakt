#!/usr/bin/env bash
# The simulation interface, in a real browser, against a real API.
#
# Brings up PostgreSQL and the API, creates a company, a user and a model, then
# hands a headless Chromium the URL and asserts on what a person would see.
# The claim it exists to establish is the one no other suite can: that the
# screen is wired to the backend and not to a fixture (section 24).
#
# Usage: tests/e2e/simulation-ui.sh
# Requires: PostgreSQL, cargo, curl, node with playwright, chromium.
...
if [[ "${EUID:-$(id -u)}" -eq 0 && -z "${SKATTJAKT_PG_REEXEC:-}" ]]; then
    ...
    exec su -s /bin/bash "$RUNAS" -c \
        "SKATTJAKT_PG_REEXEC=1 PGBIN='${PGBIN:-}' PLAYWRIGHT_MODULE='${PLAYWRIGHT_MODULE:-}' \
...
node "$ROOT/tests/e2e/simulation-ui.mjs" \
